...
const InputForm2 = ({
  ...
  return (
    ...
    <div className="parent flex flex-col bg-black text-white ">
      <div className="Input_field flex space-x-4 justify-center items-center bg-white w-[40%] text-black rounded-md p-2">
        <input
          type="text"
          placeholder={isLoading ? "Generating . . ." : "Type your prompting here..."}
          value={input}
          disabled={isLoading}
        ...
        <button
          type="submit"
          className="rounded-md shadow-md border flex flex-row justify-center items-center text-[#929292]"
          >
          {isLoading ? (
            <Loader2
            onClick={stop}
            className="p-3 h-10 w-10 bg-[#51DA4C] animate-spin"
            />
        ) : (
            <MoveRight className="p-3 h-10 w-10  bg-[#51DA4C] text-white rounded-md" />
        )}
        </button>
      </div>
...
{/* <form
  ...
  className="w-full flex-grow flex flex-row gap-2 items-end h-full mt-5 bottom-0"
>
